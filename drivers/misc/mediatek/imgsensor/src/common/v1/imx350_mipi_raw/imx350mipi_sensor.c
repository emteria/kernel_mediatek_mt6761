...
{
{ 5184, 3880, 0, 0, 5184, 3880, 2592, 1936,	0000, 0000, 2592, 1936, 0, 0, 2592, 1936},	/* Preview */
{ 5184, 3880, 0, 0, 5184, 3880, 5184, 3880,	0000, 0000, 5184, 3880, 0, 0, 5184, 3880},	/* capture */
{ 5184,  3880,  0,  0,  5184,  3880,  2592,  1936,  0000,  0000,  2592,  1936,  0,  0,  2592,  1936}, // video
{ 5184,  3880,  0,  0,  5184,  3880,  2592,  1936,  0000,  0000,  2592,  1936,  0,  0,  2592,  1936}, //hight speed video
{ 5184,  3880,  0,  0,  5184,  3880,  2592,  1936,  0000,  0000,  2592,  1936,  0,  0,  2592,  1936},// slim video
};
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
   ...
   LOG_INF("imx350 gain =%d, reg_gain =%d\n", gain,reg_gain);
...
static void sensor_init(void)
{
	...
	write_cmos_sensor_8(0x3C7F, 0x09);
	...
	write_cmos_sensor_8(0x9004, 0x0C);
	...
	write_cmos_sensor_8(0xAA20, 0x20);
	write_cmos_sensor_8(0xAA21, 0x20);
	write_cmos_sensor_8(0xAA22, 0x20);
	write_cmos_sensor_8(0xAA23, 0x15);
	...
	write_cmos_sensor_8(0xAA69, 0x03);
	write_cmos_sensor_8(0xAA6B, 0x03);
	write_cmos_sensor_8(0xAA6D, 0x03);
	write_cmos_sensor_8(0xAA6F, 0x06);
	...
	write_cmos_sensor_8(0xB6D9, 0x30);
...
static void preview_setting(void)
{
	...
	write_cmos_sensor_8(0x0340, 0x20);
	...
	write_cmos_sensor_8(0x034B, 0x1F);
	...
	write_cmos_sensor_8(0x0820, 0x20);
	write_cmos_sensor_8(0x0821, 0xD0);
	...
	write_cmos_sensor_8(0x3F79, 0x3E);
	write_cmos_sensor_8(0x3F7A, 0x00);
	write_cmos_sensor_8(0x3F7B, 0x00);
	...
	write_cmos_sensor_8(0x0202, 0x20);
...
static void capture_setting(kal_uint16 currefps, kal_bool stream_on)
{
	...
	write_cmos_sensor_8(0x0341, 0x9C);
	write_cmos_sensor_8(0x0344, 0x00);	
	write_cmos_sensor_8(0x0345, 0x00);	
	write_cmos_sensor_8(0x0346, 0x00);	
	write_cmos_sensor_8(0x0347, 0x00);	
	...
	write_cmos_sensor_8(0x5617, 0x32);	
	write_cmos_sensor_8(0x9104, 0x04);	
	write_cmos_sensor_8(0x0202, 0x11);
	write_cmos_sensor_8(0x0203, 0x88);
	write_cmos_sensor_8(0x0224, 0x01);	
	write_cmos_sensor_8(0x0225, 0xF4);	
	write_cmos_sensor_8(0x0204, 0x00);	
	write_cmos_sensor_8(0x0205, 0x00);	
		...
		do {
			*sensor_id = ((read_cmos_sensor_8(0x0016) << 8) | read_cmos_sensor_8(0x0017));
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("i2c write id: 0x%x, 350sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			
				return ERROR_NONE;
			}
			LOG_INF("Read sensor id fail, i2c_write_id: 0x%x .350sensor_id: 0x%x \n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
            sensor_id = ((read_cmos_sensor_8(0x0016) << 8) | read_cmos_sensor_8(0x0017));
			if (sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("i2c write id: 0x%x,open350 sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
				break;
			}
