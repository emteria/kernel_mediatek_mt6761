...
int mir3da_chip_resume(MIR_HANDLE handle)
{
	...
	if(gsensor_chip_info.asic==ASIC_2513A){
		...
		if((reg_data == 0x55)||(reg_data == 0x50)){
			...
			mir3da_register_mask_write(handle, NSA_REG_POWERMODE_BW, 0xff, 0x3c);
	...
	if(is_da217 == 1){
		res = mir3da_step_count_init(handle);
		if(res){
...
int mir3da_step_count_init(MIR_HANDLE handle){
	int res = 0;
	// step detecter irq init
	res |= mir3da_register_mask_write(handle, NSA_REG_ODR_AXIS_DISABLE,   0xff, 0x07);
	res |= mir3da_register_mask_write(handle, NSA_REG_G_RANGE,            0xff, 0x00);
...
int mir3da_set_step_enable(MIR_HANDLE handle, char enable)
{
	...
	if(enable){
		res |= mir3da_register_mask_write(handle, NSA_REG_STEP_FILTER, 0x80, 0x80);	//step enable bit
