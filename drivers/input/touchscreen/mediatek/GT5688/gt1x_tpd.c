...
static char tp_modules[][MODULE_MAX_LEN]={
	[MODULE_SENSOR_ID0]  = MODULE_NAME_0,	
	[MODULE_SENSOR_ID1]  = MODULE_NAME_1,
	[MODULE_SENSOR_ID2]  = MODULE_NAME_2,
	[MODULE_SENSOR_ID3]  = MODULE_NAME_3,
	[MODULE_SENSOR_ID4]	 = MODULE_NAME_4,
	[MODULE_SENSOR_ID5]  = MODULE_NAME_5,
...
//#endif
	...
	get_module_name(gt1x_version.sensor_id,module_name_g);
        ...
        #ifdef CONFIG_WTK_CQ_HARDWARE_INFO_FORMAT
            len=sprintf(tp_info,"TP IC:GT%s,TP MODULE:%s,SW FirmWare:0x%06X,Sample FirmWare:0x%06X",gt1x_version.product_id,module_name_g,gt1x_version.patch_id,RGK_SAMPLE_VERSION);
        #else
            len=sprintf(tp_info,"TP IC:GT%s,TP MODULE:%s,TP I2C ADR:0x%x,SW FirmWare:0x%06X,Sample FirmWare:0x%06X",gt1x_version.product_id,module_name_g,client->addr,gt1x_version.patch_id,gt1x_version.patch_id);
        #endif
	#else
        #ifdef CONFIG_WTK_CQ_HARDWARE_INFO_FORMAT
        ...
        #else
        len=sprintf(tp_info,"TP IC:GT%s,TP MODULE:%s,SW FirmWare:0x%06X",gt1x_version.product_id,module_name_g,gt1x_version.patch_id);
        #endif
	#endif
	//fix_tp_proc_info(tp_info,len);
	//add for 9375 info end

	GTP_INFO(" tpd_i2c_probe end.\n");
	tpd_load_status = 1;
