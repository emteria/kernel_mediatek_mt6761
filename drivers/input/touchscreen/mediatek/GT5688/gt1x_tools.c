...
//	memcpy(buf, cmd_head.flag_addr, cmd_head.addr_len);
	...
	if(copy_from_user(buf, cmd_head.flag_addr, cmd_head.addr_len))
...
static ssize_t gt1x_tool_write(struct file *filp, const char __user * buff, size_t len, loff_t * data)
{
	...
	if (1 == cmd_head.wr) {
		...
		if(ret){
			GTP_DEBUG("copy_from_user fail cmd_head.wr == 15");
		}
	...
	if (cmd_head.wr % 2) {
		...
		*ppos += sizeof(IC_TYPE);
		...
		if(copy_to_user(buffer,progress_buf,4)){
			GTP_DEBUG("copy_to_user failed 440");
			return -1;
		}
		*ppos += 4;
		return 4;
		...
		{
			GTP_DEBUG("copy_to_user failed 455");
			return -1;
		}
		//buffer[tmp_len] = 0;
		*ppos += tmp_len + 1;
